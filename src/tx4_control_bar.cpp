...
tx4_control_bar::tx4_control_bar(const int &left_padding, const bool &master, QWidget *parent)
	: QWidget(parent) {

	m_masterBar = master;
	i_barSteps = 59;
	i_leftPadding = left_padding;
	...
	this->setStyleSheet(m_masterBar ? controlBarBodyStyle : controlBarBodyStyleTransparent);
...
void tx4_control_bar::initTimeline() {
	...
	w_timelineContainer->setStyleSheet(m_masterBar ? buttonContainerBodyStyle : buttonContainerBodyStyleTransparent);
	QHBoxLayout *v_timelineContainerLayout = new QHBoxLayout(w_timelineContainer);
	Util::setLayoutZero(v_timelineContainerLayout);
	v_timelineContainerLayout->setContentsMargins(i_leftPadding,0,0,0);
	...
	l_timeCurrentLabel = new tx4_label("00:00", 10, timeLabelStyle, QFont::Medium, Qt::AlignCenter);
	v_timeContainerLayout->addWidget(l_timeCurrentLabel);

	if (m_masterBar) {
		l_timeTotalLabel = new tx4_label("00:59", 10, timeLabelStyle, QFont::Medium, Qt::AlignCenter);
		v_timeContainerLayout->addWidget(new tx4_label("/", 10, dividerLabelStyle, QFont::Medium, Qt::AlignCenter));
...
// SLOTS: {
	void tx4_control_bar::on_timelineValueUpdate(int value) {
		QString timeValue = QString("00:") + QString::number(value/TIMELINE_MAX);
